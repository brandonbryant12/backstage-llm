...
import { amazonQBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [amazonQBackendPlugin],
    });

    await request(server).get('/api/amazon-q-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/amazon-q-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/amazon-q-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/amazon-q-backend/todos/${createdTodoItem.id}`)
